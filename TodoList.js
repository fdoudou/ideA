...
function TodoList(props) {
    ...
    const TdList = todo.map((td,index) => {
        return (
            <>
                {/* <Link > */}
                    <div key={td.id} style={{background:"skyblue",borderRadius:'3px',height:'140px',width:'140px',textAlign:'center',verticalAlign:'middle',margin:'5px'}}>
                        <div >
                            <button className="del-btn" onClick={()=>{removeTask(index)}}>&times;</button>
                            <span>{td.title}</span>
                            <div>
